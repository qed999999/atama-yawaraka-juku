...
const ALL_QUESTIONS: Question[] = [
  { hint: "モーと なく どうぶつ", emoji: "🐄", correct: "うし", choices: ["うし", "うま", "ぶた", "ひつじ"] },
  { hint: "ニャーと なく どうぶつ", emoji: "🐱", correct: "ねこ", choices: ["ねこ", "いぬ", "きつね", "うさぎ"] },
  { hint: "ワンと なく どうぶつ", emoji: "🐶", correct: "いぬ", choices: ["いぬ", "ねこ", "たぬき", "おおかみ"] },
  { hint: "くびが とても ながい どうぶつ", emoji: "🦒", correct: "きりん", choices: ["きりん", "ぞう", "うま", "らくだ"] },
  { hint: "はなが ながい どうぶつ", emoji: "🐘", correct: "ぞう", choices: ["ぞう", "きりん", "かば", "さい"] },
  { hint: "もりの おうさま ライオンは なに いろ？", emoji: "🦁", correct: "きいろっぽい", choices: ["きいろっぽい", "あか", "しろ", "くろ"] },
  { hint: "かわにいる からだが おおきい どうぶつ", emoji: "🦛", correct: "かば", choices: ["かば", "ぞう", "わに", "あざらし"] },
  { hint: "ポーポーと なく とり", emoji: "🐦", correct: "はと", choices: ["はと", "からす", "すずめ", "つばめ"] },
  { hint: "みずの なかで およぐ どうぶつ", emoji: "🐬", correct: "イルカ", choices: ["イルカ", "うさぎ", "たぬき", "きつね"] },
  { hint: "8ほんの あしを もつ どうぶつ", emoji: "🐙", correct: "たこ", choices: ["たこ", "いか", "かに", "えび"] },
  { hint: "ゆっくり うごく からを もつ どうぶつ", emoji: "🐢", correct: "かめ", choices: ["かめ", "かに", "かえる", "とかげ"] },
  { hint: "じゃんぷが とくいな どうぶつ", emoji: "🦘", correct: "カンガルー", choices: ["カンガルー", "きりん", "さる", "うさぎ"] },
  { hint: "さかなを つかまえる くちばしが ながい とり", emoji: "🦩", correct: "つる", choices: ["つる", "にわとり", "すずめ", "からす"] },
  { hint: "しろくろの もよう の どうぶつ", emoji: "🐼", correct: "パンダ", choices: ["パンダ", "しろくま", "ぺんぎん", "シマウマ"] },
  { hint: "のこぎりのような くちを もつ さかな", emoji: "🦈", correct: "サメ", choices: ["サメ", "マグロ", "イルカ", "タイ"] },
  { hint: "たまごを うむ どうぶつ は どれ？", emoji: "🐔", correct: "にわとり", choices: ["にわとり", "ねこ", "うし", "いぬ"] },
  { hint: "きを たべる からだが おおきい どうぶつ", emoji: "🐘", correct: "ぞう", choices: ["ぞう", "うし", "うま", "かば"] },
  { hint: "ペタペタと ひれで あるく どうぶつ", emoji: "🐧", correct: "ペンギン", choices: ["ペンギン", "あひる", "つる", "はと"] },
  { hint: "はちみつが だいすきな どうぶつ", emoji: "🐻", correct: "くま", choices: ["くま", "さる", "リス", "きつね"] },
  { hint: "きのぼりが とくいな どうぶつ", emoji: "🐒", correct: "さる", choices: ["さる", "ねこ", "りす", "コアラ"] },
];
...
export default function AnimalQuiz({ onExit, onScore, prevBest, unit = "てん" }: Props) {
  ...
  function pick(choice: string) {
    ...
    timerRef.current = window.setTimeout(() => {
      ...
      setQIdx(i => {
        let next = i + 1;
        if (pool.length > 1 && pool[next % pool.length].correct === prevAnswerRef.current) next++;
        prevAnswerRef.current = pool[next % pool.length].correct;
        return next;
      });
    }, FEEDBACK_MS);
  }
  ...
  if (phase === "ready") return (
    <KidShell title="どうぶつクイズ" onExit={onExit}>
  ...
  return (
    <KidShell title="どうぶつクイズ" onExit={onExit} rightExtra={<button style={kidSecondaryBtn} onClick={goResult}>けっかへ</button>}>
      <div style={screenCol}>
        <div style={kidTopBar}>
        ...
        <div style={{ ...kidPanelFlex, justifyContent: "space-between" }}>
          ...
          <div style={{
            ...
            fontSize: "clamp(15px, 4.2vw, 20px)", lineHeight: 1.5, padding: "0 4px",
          }}>
            {q?.hint}
          </div>
              ...
              return (
                <button key={c} onClick={() => pick(c)} disabled={!!feedback}
                  style={{
                    ...
                    fontWeight: 1000, fontSize: "clamp(14px, 3.8vw, 17px)",

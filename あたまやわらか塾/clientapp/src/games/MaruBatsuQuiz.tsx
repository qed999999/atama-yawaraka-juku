...
type Question = { text: string; answer: boolean };

const QUESTIONS: Question[] = [
  { text: "ぞうは ちきゅうで いちばん おおきい どうぶつ", answer: false },
  { text: "たいようは ひがしから のぼる", answer: true },
  { text: "さかなは みず の なかで いきをする", answer: true },
  { text: "にほんの こっき は あかと しろ", answer: true },
  { text: "ペンギンは そらを とぶことが できる", answer: false },
  { text: "にんじんは くだもの", answer: false },
  { text: "ちきゅうは まるい かたち", answer: true },
  { text: "ライオンは くさを たべる どうぶつ", answer: false },
  { text: "1＋1＝3", answer: false },
  { text: "5＋5＝10", answer: true },
  { text: "ねこは いぬ の なかま", answer: false },
  { text: "いちごは あかい いろ", answer: true },
  { text: "きりんの くびは みじかい", answer: false },
  { text: "あさひが でるのは あさ", answer: true },
  { text: "バナナの かわは あかい", answer: false },
  { text: "みずは 100ど で こおる", answer: false },
  { text: "ちょうちょは むし の なかま", answer: true },
  { text: "ねこは みずが だいすき", answer: false },
  { text: "ひよこは とりの こども", answer: true },
  { text: "くじらは さかな", answer: false },
  { text: "りんごは きから なる", answer: true },
  { text: "2×3＝7", answer: false },
  { text: "3×3＝9", answer: true },
  { text: "たこには あしが 8ほん ある", answer: true },
  { text: "へびには あしが ある", answer: false },
];
...
export default function MaruBatsuQuiz({ onExit, onScore, prevBest, unit = "てん" }: Props) {
  ...
  const [feedback, setFeedback] = useState<{ ok: boolean; picked: boolean } | null>(null);
  ...
  function pick(answer: boolean) {
    ...
    timerRef.current = window.setTimeout(() => {
      ...
      setQIdx(i => {
        let next = i + 1;
        const currentAnswer = String(pool[(i) % pool.length].answer);
        if (pool.length > 1 && String(pool[next % pool.length].answer) === currentAnswer) next++;
        prevAnswerRef.current = String(pool[next % pool.length].answer);
        return next;
      });
    }, FEEDBACK_MS);
  }
  ...
  if (phase === "ready") return (
    <KidShell title="○×クイズ" onExit={onExit}>
      <div style={{ ...kidPanel, textAlign: "left" }}>
        <div style={{ fontSize: 48, marginBottom: 8 }}>⭕❌</div>
        <div style={{ fontSize: 20, fontWeight: 1000, marginBottom: 8 }}>
          ただしければ ○、まちがいなら × を おそう！
        </div>
        <div style={{ fontSize: 15, fontWeight: 900, opacity: 0.8, marginBottom: 16 }}>
          60びょうで なんもん こたえられるかな？
        </div>
  ...
  return (
    <KidShell title="○×クイズ" onExit={onExit} rightExtra={<button style={kidSecondaryBtn} onClick={goResult}>けっかへ</button>}>
      <div style={screenCol}>
        <div style={kidTopBar}>
        ...
        <div style={{ ...kidPanelFlex, justifyContent: "space-between" }}>
          ...
          <div style={{
            ...
            fontSize: "clamp(16px, 4.5vw, 22px)", fontWeight: 1000, textAlign: "center",
            lineHeight: 1.5, padding: "8px 4px",
          }}>
            {q?.text}
          </div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
            <button
              ...
              style={{
                ...maruBtn,
                opacity: feedback && !feedback.ok && feedback.picked ? 0.5 : 1,
                border: feedback && feedback.picked && feedback.ok ? "4px solid #22c55e"
                  : feedback && !feedback.picked && q.answer ? "4px solid #22c55e"
            ...
            <button
              ...
              style={{
                ...batsuBtn,
                opacity: feedback && feedback.picked === false && feedback.ok ? 0.5 : 1,
                border: feedback && !feedback.picked && feedback.ok === false ? "4px solid #22c55e"
                  : feedback && feedback.picked === false && !feedback.ok ? "4px solid #ef4444"
...
const maruBtn: React.CSSProperties = {
  fontSize: "clamp(48px, 14vw, 72px)", borderRadius: 22,
  background: "rgba(34,197,94,0.08)", cursor: "pointer",
  padding: "12px", lineHeight: 1, transition: "border 0.2s",
  aspectRatio: "1",
};
const batsuBtn: React.CSSProperties = {
  fontSize: "clamp(48px, 14vw, 72px)", borderRadius: 22,

...
type Question = { items: string[]; oddIdx: number; hint: string };

const GROUPS: Array<{ members: string[]; label: string }> = [
  { members: ["🐶", "🐱", "🐰", "🐻", "🐼", "🐨"], label: "どうぶつ" },
  { members: ["🍎", "🍊", "🍋", "🍇", "🍓", "🍑"], label: "くだもの" },
  { members: ["🥕", "🥦", "🧅", "🍆", "🌽", "🥒"], label: "やさい" },
  { members: ["🚗", "🚕", "🚙", "🏎️", "🚌", "🚎"], label: "くるま" },
  { members: ["✈️", "🚀", "🛸", "🚁", "🛩️"], label: "そらとぶのりもの" },
  { members: ["⚽", "🏀", "🎾", "⚾", "🏈", "🏐"], label: "ボール" },
  { members: ["🍕", "🍔", "🌮", "🌯", "🥗", "🥙"], label: "たべもの" },
  { members: ["🌹", "🌷", "🌸", "💐", "🌺", "🌻"], label: "はな" },
  { members: ["🎸", "🎹", "🎺", "🎻", "🥁", "🪗"], label: "がっき" },
  { members: ["📱", "💻", "🖥️", "⌨️", "🖱️"], label: "でんしきき" },
  { members: ["☀️", "🌙", "⭐", "🌟", "🌈", "⚡"], label: "てんき・そら" },
  { members: ["🏠", "🏡", "🏢", "🏣", "🏤", "🏥"], label: "たてもの" },
];
...
function makeQuestion(prev = ""): Question {
  ...
  let tries = 0;
  do {
    // Pick a correct group (3 members) and one from a different group
    const gIdx = Math.floor(Math.random() * GROUPS.length);
    const group = GROUPS[gIdx];
    const members = shuffle([...group.members]).slice(0, 3);

    // Pick odd one from a different group
    let otherGIdx = gIdx;
    while (otherGIdx === gIdx) otherGIdx = Math.floor(Math.random() * GROUPS.length);
    const odd = GROUPS[otherGIdx].members[Math.floor(Math.random() * GROUPS[otherGIdx].members.length)];

    const items = shuffle([...members, odd]);
    const oddIdx = items.indexOf(odd);
    result = { items, oddIdx, hint: group.label };
    tries++;
  } while (result.items[result.oddIdx] === prev && tries < 20);
  return result!;
}

export default function OddOneOut({ onExit, onScore, prevBest, unit = "てん" }: Props) {
  ...
  const start = useCallback(() => {
    ...
    prevAnswerRef.current = q.items[q.oddIdx];
    setQuestion(q);
    setPhase("playing");
  }, []);
  ...
  if (phase === "ready") return (
    <KidShell title="なかまはずれ" onExit={onExit}>
      <div style={{ ...kidPanel, textAlign: "left" }}>
        <div style={{ fontSize: 48, marginBottom: 8 }}>🔍</div>
        <div style={{ fontSize: 20, fontWeight: 1000, marginBottom: 8 }}>なかまじゃない ひとつを えらぼう！</div>
        <div style={{ fontSize: 15, fontWeight: 900, opacity: 0.8, marginBottom: 16, lineHeight: 1.7 }}>
          3つは なかまだけど、1つだけ ちがうよ<br />どれが なかまはずれかな？
        </div>
  ...
  return (
    <KidShell title="なかまはずれ" onExit={onExit} rightExtra={<button style={kidSecondaryBtn} onClick={goResult}>けっかへ</button>}>
      <div style={screenCol}>
        <div style={kidTopBar}>
        ...
        <div style={{ ...kidPanelFlex, alignItems: "center", justifyContent: "space-between" }}>
          <div style={{ textAlign: "center" }}>
            <div style={{ fontWeight: 1000, fontSize: 16 }}>なかまじゃない ひとつは？</div>
          </div>

          {/* 2x2 grid */}
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, width: "min(300px, 90vw)" }}>
            {question.items.map((item, i) => {
              const isOdd = i === question.oddIdx;
              const isPicked = feedback?.picked === i;
              let bg = "rgba(255,255,255,0.95)";
              let border = "3px solid rgba(0,0,0,0.08)";
              if (feedback) {
                if (isPicked && feedback.ok) { bg = "rgba(34,197,94,0.15)"; border = "3px solid #22c55e"; }
                else if (isPicked && !feedback.ok) { bg = "rgba(239,68,68,0.12)"; border = "3px solid #ef4444"; }
                else if (isOdd) { bg = "rgba(34,197,94,0.15)"; border = "3px solid #22c55e"; }
              }
              return (
                <button key={i} onClick={() => pick(i)} disabled={!!feedback}
                  style={{ aspectRatio: "1", borderRadius: 22, border, background: bg,
                    display: "grid", placeItems: "center",
                    fontSize: "clamp(36px, 12vw, 56px)", lineHeight: 1,
          ...
          <div style={kidFeedbackCard(feedback.ok)}>
            ...
            {!feedback.ok && (
              <div style={{ fontSize: 16, opacity: 0.85, marginTop: 6 }}>
                こたえ：{question.items[question.oddIdx]}
              </div>
            )}
